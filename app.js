        ...
        operation: [
          {
            ...
            id: "btn-hextorgb",
            noOfInputs: 1,
            placeholder: ["Enter Hex Value eg:#0a0a0a"],
          },
        ],
        id: "hextorgb",
      },
        ...
        operation: [
          {
            ...
            id: "btn-humantoepoch",
            noOfInputs: 1,
            placeholder: ["Enter Date eg: 2021,5,11"],
          },
        ],
        id: "humantoepoch",
      },
...
const appLists = [
  ...
  {
    id: "number-base",
    path: "./number-base.html",
    name: "Number Base Converter",
  },
  {
    id: "text-tool-string",
    path: "./text-tool-string.html",
    name: "Text Tools/String Utilities",
  },
];
...
appLists.forEach((list) => {
  ...
  appListLink.setAttribute("href", list.path);
  appListLink.id = list.id;
  appListLink.className = "list-group-item list-group-item-action";
  appListLink.innerText = list.name;
  listGroup.appendChild(appListLink);
});

window.onload = () => {
  appLists.forEach((list) => {
    const selectedApp = window.location.pathname.split("/")[1];
    if (window.location.pathname.split("/")[1].includes(list.id)) {
      document.getElementById(list.id).classList.add("active");
      const selectedUtilityId = selectedApp.split(".")[0];
      const filteredUtilityApp = utilityApp.filter(
        (app) => app.id === selectedUtilityId
      );
...
defaultMessage.innerText = "Select any one option";
...
const constructSelectedApp = (appId, operations) => {
  ...
  for (let operation of filteredApp[0].operation) {
    ...
    btn.id = operation.id;
    btn.className = "btn btn-outline-primary btn-primary-spacing";
...
const operationFunc = (value, id) => {
  switch (id) {
    ...
    case "btn-decode-url":
      decodeUrl(value);
      break;
    case "btn-base64-encode":
      base64Encode(value);
      break;
    case "btn-base64-decode":
      base64Decode(value);
      clearInput();
      break;
    case "btn-md5":
    ...
    case "btn-hextorgb":
      hextorgb(value);
      break;
    case "btn-epochtohuman":
      epochToHumanDate(value);
      break;
    case "btn-humantoepoch":
      humanToEpochTime(value);
      break;
    case "btn-mtokm":
      meterToKilometer(value);
      break;
    case "btn-cmtokm":
      centimeterToKilometer(value);
      break;
    case "btn-kmtomile":
      kilometerToMile(value);
      break;
    case "btn-binarytodecimal":
      binaryToDecimal(value);
      break;
    case "btn-binarytohex":
      binaryToHex(value);
      break;
    case "btn-binarytooctal":
      binaryToOctal(value);
      break;
    case "btn-decimaltobinary":
      decimalToBinary(value);
      break;
    case "btn-decimaltohex":
      decimalToHex(value);
      break;
    case "btn-decimaltooctal":
      decimalToOctal(value);
      break;
    case "btn-hextobinary":
      hexToBinary(value);
      break;
    case "btn-hextodecimal":
      hexToDecimal(value);
      break;
    case "btn-hextooctal":
      hexToOctal(value);
      break;
    case "btn-octaltobinary":
      octalToBinary(value);
      break;
    case "btn-octaltodecimal":
...
const epochToHumanDate = (value) => {
  ...
  document.querySelector(
    ".output"
  ).innerText = `{ Year : ${humanDate.getFullYear()}, Month : ${humanDate.getMonth()}, Date : ${humanDate.getDate()}, Hours : ${humanDate.getHours()}, Minutes : ${humanDate.getMinutes()}, Seconds: ${humanDate.getSeconds()}}`;
  clearInput();
};
...
const rgbtohex = () => {
  ...
  let convertedValue =
    ...
    ((1 << 24) + (Number(red) << 16) + (Number(green) << 8) + Number(blue))
      .toString(16)
      .slice(1);
  document.querySelector(".output").innerText = convertedValue;
  clearInput();
};
...
const hextorgb = (value) => {
  let convertedValue =
    "rgba(" +
    (value = value.replace("#", ""))
      .match(new RegExp("(.{" + value.length / 3 + "})", "g"))
      .map(function (l) {
        return parseInt(value.length % 2 ? l + l : l, 16);
      })
      .join(",") +
    ")";
  document.querySelector(".output").innerText = convertedValue;
...
const decimalToHex = (value) => {
  document.querySelector(".output").innerText = parseInt(value, 10).toString(
    16
  );
  clearInput();
};
...
const hexToDecimal = (value) => {
  document.querySelector(".output").innerText = parseInt(value, 16).toString(
    10
  );
  clearInput();
};
